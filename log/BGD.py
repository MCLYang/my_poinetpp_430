        ...
        for file in data_list:
            with h5py.File(os.path.join(root, file),'r') as f:
                try:
                    ...
                    current_point = []
                    current_lable = []
                    if (self.is_validation == False and self.is_test == False):
                    ...
                    elif(self.is_validation == True and self.is_test == False):
                        ...
                        current_point.append(np.array(f['laserID'][train_tail:validation_tail,:]).reshape(n_frame,n_points,1))
                        current_point.append(np.array(f['intensity'][train_tail:validation_tail,:]).reshape(n_frame,n_points,1))
                        current_point = np.concatenate(current_point,axis = 2)
                        lableset.append(np.array(f['label'][train_tail:validation_tail,:]))
                        pointset.append(current_point)

                    elif(self.is_validation == False and self.is_test == True):
                        ...
                        current_point.append(np.array(f['laserID'][validation_tail:test_tail,:]).reshape(n_frame,n_points,1))
                        current_point.append(np.array(f['intensity'][validation_tail:test_tail,:]).reshape(n_frame,n_points,1))
                        current_point = np.concatenate(current_point,axis = 2)
